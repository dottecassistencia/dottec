/*
  ...
  Inclua em todas as páginas antes de </head>:
    <script>
      const PAGE_ID = 'os';
      const BREADCRUMB = [{label:'OS', href:'os.html'}];
    </script>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  style.textContent = `
    ...
    .dnt-nav-item { height: 100%; display: flex; align-items: center; gap: 5px; padding: 0 13px; font-size: 12px; font-weight: 600; color: #8A887F; white-space: nowrap; text-decoration: none; border-bottom: 2px solid transparent; transition: color .15s, border-color .15s, background .15s; flex-shrink: 0; position: relative; }
    ...
    .dnt-nav-badge { background: #E8604A; color: #fff; font-size: 8.5px; font-weight: 800; border-radius: 10px; padding: 1px 5px; line-height: 1.4; position: absolute; top: 8px; right: 4px; }
    ...
    #dnt-search-row { display: flex; align-items: center; gap: 8px; height: 42px; padding: 0 14px; cursor: text; }
    #dnt-search-row i { font-size: 15px; color: #B5B2A8; flex-shrink: 0; }
    #dnt-search-inp { flex: 1; border: none; outline: none; font-size: 13.5px; background: transparent; color: #1C1C1A; font-family: inherit; text-transform: uppercase; caret-color: #29B8C2; }
    ...
    #dnt-sr { display: none; position: absolute; top: calc(100% + 2px); left: 0; right: 0; background: #fff; border: 1px solid #EAE7DF; border-radius: 12px; box-shadow: 0 16px 48px rgba(0,0,0,.13); max-height: 460px; overflow-y: auto; z-index: 999; }
    ...
    .dnt-sr-subr:hover, .dnt-sr-subr.foc { background: #F5F4F0; }
  ...
  // breadcrumb
  const crumbParts = [{ label: 'Início', href: 'dashboard.html' }, ...CRUMB];
  const crumbHTML = crumbParts.map((c, i) => {
  ...
  const navHTML = MENU.map(m => `
    ...
    ${['os','orcamentos','cadastro'].includes(m.id) ? '<div class="dnt-nav-sep"></div>' : ''}
  `).join('');
    ...
    <div id="dnt-bar">
      ...
      <div id="dnt-search-wrap">
        <div id="dnt-search-row">
          ...
          <input id="dnt-search-inp" type="text" placeholder="Buscar… (Ctrl+K)" autocomplete="off" autocorrect="off" spellcheck="false"/>
          <span id="dnt-search-kbd">Ctrl+K</span>
        </div>
        <div id="dnt-sr"></div>
      </div>
    </div>
  ...
  let RESULTADOS = [], FOCI = -1, busTimer = null;
  ...
  document.addEventListener('click', e => {
    if (!e.target.closest('#dnt-search-wrap')) {
      sr.classList.remove('open');
      FOCI = -1;
    }
  });

  // Ctrl+K abre busca — ÚNICO atalho de teclado, não interfere com nada
  ...
  // navegação por teclado DENTRO da busca
  inp.addEventListener('keydown', e => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); FOCI = Math.min(FOCI + 1, rows.length - 1); }
      ...
      e.preventDefault();
      const r = rows[FOCI];
      if (r) { const i = +r.dataset.i; if (RESULTADOS[i]) location.href = RESULTADOS[i].url; }
    }
  ...
  async function executaBusca(q) {
    ...
    if (pessoas.length) {
      ...
      for (const p of pessoas) {
        ...
        const [rOp, rCp] = await Promise.all([
          safe(db.from('ordens_servico').select('id,numero,status,aparelho').eq('pessoa_id', p.id).limit(2)),
          safe(db.from('contas').select('id,tipo,descricao,valor').eq('pessoa_id', p.id).in('status', ['ABERTA', 'VENCIDA']).limit(2)),
        ]);
        const subs = [
    ...
    if (!RESULTADOS.length) {
      html = `<div class="dnt-sr-empty"><i class="ti ti-search-off"></i>Sem resultados para "${q.toUpperCase()}"</div>`;
    } else {
      html += '<div class="dnt-sr-foot">↑ ↓ navegar · Enter abrir · ESC fechar</div>';
    }
    sr.innerHTML = html;
  }
